...
public class Ellipsoid extends UVShape
{
    ...
    public void evalPosition(double u, double v, Point3d p)
    {
    	...
    	x = ax.value * Math.cos(u) * Math.cos(v);
    	y = ay.value * Math.sin(u) * Math.cos(v);
    	z = az.value * Math.sin(v);    	
    ...
    public void evalNormal(double u, double v, Vector3d n)
    {
    	...
    	x = ay.value * az.value * Math.cos(u) * Math.cos(v);
    	y = ax.value * az.value * Math.sin(u) * Math.cos(v);
    	z = ax.value * ay.value * Math.sin(v);    	
