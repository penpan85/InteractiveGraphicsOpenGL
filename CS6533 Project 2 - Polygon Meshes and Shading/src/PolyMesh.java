...
public class PolyMesh extends Shape
{
    ...
    private class VertexPM implements Vertex
    {
        ...
        public void setNormal(Vector3d newNormal) { normal = newNormal; }
    }
    ...
    {
    	// for each polygon i --> compute normal (normalized) of i
    	for (int i = 0; i < polygons.length; i++)
        {
        	// calculating a single normal for the polygon
        	polygons[i].computeNormal();
        	Vector3d normal = polygons[i].getNormal();
        	normal.normalize();
        }
    	
    	// for each vertex j --> set (n_avg)_j = 0
    	for (int j = 0; j < vertices.length; j++)
    	{
    		VertexPM v = (VertexPM)vertices[j];
    		v.setNormal(new Vector3d(0,0,0));
    	}
    	
    	
    	for (int k = 0; k < polygons.length; k++)
    	{
    		for (int v = 0; v < polygons[k].size(); v++)
    		{
    			VertexPM w = (VertexPM)polygons[k].getVertex(v);
    			
    			Vector3d nAvg = new Vector3d();
    			nAvg.add(w.getNormal());
    			nAvg.add(polygons[k].getNormal());
    	...
    	{
    		Vector3d normal = vertices[m].getNormal();
    		normal.normalize();
    	}
    }
