...
public abstract class Shape
{
    ...
    private void drawPolygons(GL gl)
    {
        ...
        if (drawSmooth.value) {
        	...
        	drawPolygonsWithVertexNormals(gl);
    ...
    private void drawWireframe(GL gl)
    {
        ...
        gl.glColor3f(0.25f, 0.5f, 0.66f);                
        drawAllPolygonsSimple(gl);
    }
    
    private void drawSilhouette(GL gl)
    {
    	// draw the polygons in a solid color matching background color
    	gl.glPolygonOffset(1,1);
    	gl.glEnable(GL.GL_POLYGON_OFFSET_FILL);
    	gl.glPolygonMode(GL.GL_FRONT_AND_BACK, GL.GL_FILL);
    	...
    	// draw back-facing polygons as thickened wireframe
    ...
    private void drawPolygonsSimple(GL gl)
    {
	    for (int i = 0; i < polygons.length; i++)
	    {
	    	// calculating a single normal for the polygon
	    	Vector3d normal = new Vector3d(polygons[i].getNormal());
	    	
	    	gl.glBegin(GL.GL_POLYGON);
	    	gl.glNormal3d(normal.x, normal.y, normal.z);
	    	drawPolygonSimple(gl, polygons[i]);
	    	gl.glEnd();
	    }
    }
    
    private void drawAllPolygonsSimple(GL gl)
    {
    	for (int i = 0; i < polygons.length; i++)
        {
        	gl.glBegin(GL.GL_POLYGON);
    ...
    // draw a single polygon, simple version
    private void drawPolygonSimple(GL gl, Polygon p)
    {
    	for (int j = 0; j < p.size(); j++)
    	{
    		Vertex v = p.getVertex(j);
    		gl.glVertex3d(v.getPoint().x, v.getPoint().y, v.getPoint().z);
    	}
    }
    
    // draw all polygons with normal at each vertex
    private void drawPolygonsWithVertexNormals(GL gl)
    {
    	for (int i = 0; i < polygons.length; i++)
    	{
        	gl.glBegin(GL.GL_POLYGON);
        	drawPolygonWithVertexNormals(gl, polygons[i]); // normals pre-calculated when the shape was loaded
        	gl.glEnd();
    	}
    }
    
    // draw a single polygon with normal of each vertex
    private void drawPolygonWithVertexNormals(GL gl, Polygon p)
    {
    	for (int j = 0; j < p.size(); j++)
    	{
    		Vertex v = p.getVertex(j);
    		gl.glNormal3d(v.getNormal().x, v.getNormal().y, v.getNormal().z);
    		gl.glVertex3d(v.getPoint().x, v.getPoint().y, v.getPoint().z);
    	}
    }
}
