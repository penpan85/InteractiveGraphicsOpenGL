...
class Material extends RaytracerObject
{
    ...
    // Bumpmap image specification
    String bumpmapFileName = null;
    RGBImage bumpmapImage = null;
    ...
    // Bumpmap image specification
    double bumpmapScaleU       = 1.0;
    double bumpmapScaleV       = 1.0;
	...
	addSpecSpecial("bumpmap", "setBumpmapSpec", "readBumpmapSpec");
    ...
    public void setup(boolean verbose)
    {
        ...
        if (bumpmapFileName != null)
        {
            try {
				if (verbose)
				  System.out.println("Loading bumpmap " + bumpmapFileName);
		
		                bumpmapImage = new RGBImage(bumpmapFileName);
		            } catch (IOException E) {
		                System.err.println("Error while reading file " + 
		                		bumpmapFileName);
		                bumpmapImage = null;
		                bumpmapFileName = null;
		        }
        }
    }
    ...
    public void setBumpmapFileName(String s) { bumpmapFileName = s; }
    ...
    public Vector3d getBumpmapNormal(double u, double v)
    {
    	if (bumpmapImage != null)
    		return getBumpmapImageNormal(u,v);
    	else
    	{
    		System.err.println("Unknown bumpmap specification");
    		return null;
    	}
    }
    ...
    // Parser
    public static Vector readBumpmapSpec(StreamTokenizer tokenizer)
        throws ParseException, IOException
    {
		Vector<Object> v = new Vector<Object>();
    ...
    public void setBumpmapSpec(Vector v)
    {
		bumpmapFileName = (String)v.elementAt(0);
		bumpmapScaleU   = ((Double)v.elementAt(1)).doubleValue();
		bumpmapScaleV   = ((Double)v.elementAt(2)).doubleValue();
    }
    ...
    public Vector3d getBumpmapImageNormal(double u, double v)
    {
    	return bumpmapImage.getSubPixel((u * bumpmapScaleU) % 1,
					(v * bumpmapScaleV) % 1);
    }
